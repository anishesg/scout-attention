...
// Causal kernel: applies lower-triangular mask (k_idx <= q_idx) before softmax.
// Positions where k_idx > q_idx get logit = -1e38f so their weight vanishes.
__global__ void naive_attention_causal_kernel(
    const float* __restrict__ Q,
    const float* __restrict__ K,
    const float* __restrict__ V,
    float* __restrict__ O,
    int B, int H, int S_q, int S_k, int D, float scale)
{
    ...
    float denom = 0.0f;
    for (int k = 0; k < S_k; ++k) {
        logits[k] = (k <= q) ? expf(logits[k] - row_max) : 0.0f;
        denom += logits[k];
    }
    ...
    const AttentionParams& params)
{
    ...
    naive_attention_causal_kernel<<<grid, block>>>(
        Q, K, V, O,
        params.batch_size,
        params.num_heads,
        params.seq_len_q,
